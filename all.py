...
def evaluate_model(training):
    ...
    model = load_model('modelSai.h5')
    preds = model.evaluate(X,y,batch_size=32)
...
def get_test_txt(training):
    X,y = get_data(training)
    print("Loading/ Evaluating model... ")
    text_file = open("test_nvid.txt", "w")
    model = load_model('modelSai.h5')

    for image_for_model in X:
...
if __name__ == '__main__':
    ...
    if args.train:
        ...
        get_test_txt(use_training_data)
